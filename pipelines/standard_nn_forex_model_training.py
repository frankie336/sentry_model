...
from ForexMastermind.ML.tools.tools import (
    SampleData, DataFrameSaver, ModelVersionService, Balance,
    FFDataLoader, ListDataFrameColumns,  DataFrameCleanerService)
...
class FeatureEngineeringService:
    ...
    def execute(self):
        ...
        y = self.data['Prediction_Correct'].copy()
...
class DataPreprocessor:
    ...
    def _balance_dataset(self, data):
        ...
        balance = Balance(data=data, column='Prediction_Correct')
    ...
    def _separate_features_and_target(self, data):
        ...
        y = data['Prediction_Correct'].copy()
    ...
    def _process_features(self, X_train, X_test, y_train):
        ...
        return X_train_res_scaled, y_train_res, X_test_scaled
    ...
    def preprocess_data(self, unprocessed_training_data):

        print(f'There are {len(unprocessed_training_data)} in the un processed data.')

        cleaner_service = DataFrameCleanerService()
        training_data = cleaner_service.find_string_columns(df=unprocessed_training_data)
        training_data = cleaner_service.remove_unnamed_index_column(df=training_data)

        training_data = self._balance_dataset(data=training_data)

        training_data = self._drop_redundant_columns(data=training_data)
        training_data = self. _drop_nan_values(data=training_data)

        features, X, y = self._separate_features_and_target(data=training_data)
        X_train, X_test, y_train, y_test = self._split_data(X, y)
        X_train_res, y_train_res, X_test_scaled = self._process_features(X_train, X_test, y_train)
        X_train_res, y_train_res = self._apply_smote(X=X_train_res, y=y_train_res)

        print(training_data)
        print(X_train_res)
        return training_data, features, X_train_res, y_train_res, X_test_scaled, y_test
...
class ModelTrainingService:
    ...
    def train_model(self):
        ...
        # Training and test set splitting is performed by data, features, X_train_res_scaled,
        # y_train_res, X_test_scaled, y_test = self.data_preprocessor.preprocess_data(data)
        # Thus self.X and self.y here are already in their pre-processed split form!
...
class ModelEvaluationService:
    def __init__(self, model, X_test, y_test, config, training_time, feature_list, model_notes='None',
                 data_version='6', fitted_samples_count=0):
        """
        Initialize the ModelEvaluationService with the model, test data, and configuration.
        """
        self.fitted_samples_count = fitted_samples_count
        self.model = model
        self.X_test = X_test
        self.y_test = y_test
        self.config = config
        self.model_version_service = ModelVersionService(config=self.config, in_google_colab=False)
    ...
    def _save_model_metadata(self, accuracy, classification_report_str):
        ...
        metadata = {
            ...
            "total_samples": self.fitted_samples_count,
            ...
            "trained_with": self.data_version,
    ...
    def _write_metadata_to_registry(self, metadata):
        ...
        model_registry_path = os.path.join(self.config.get_model_directory(), 'model_meta_model_registry.csv')
    ...
    def _save_feature_importance(self, shap_df):
        ...
        filename = f"MetaModel_v{model_version}_FeatureImportance.csv"
    ...
    def _save_evaluation_plots(self):
        ...
        # model_version = self.model_version_service.get_current_model_version()
        model_version = "1.0.0"
        plot_filename = f"MetaModelPredictor_v{model_version}_evaluation.png"
        ...
        print(f'Trained with {self.fitted_samples_count} samples.')
    ...
    def _plot_shap_summary(self, X):
        ...
        # model_version = self.config.get_current_model_version()
        model_version = "1.0.0"

        plot_filename = f"MetaModelPredictor_v{model_version}_shap.png"
...
class TrainingPipeline:
    def __init__(self, file_path, version=None):
        ...
        self.model_version_service = ModelVersionService(self.config, in_google_colab=False)
...
class TrainingPipeline:
    def __init__(self, file_path, version=None):
        ...
        self.data_preprocessor = DataPreprocessor(config=self.config, model_version_service=self.model_version_service)
    ...
    def run(self, model_notes='None', sample_size=4):
        ...
        (processed_training_data, features, X_train_res_scaled, y_train_res,
         X_test_scaled, y_test) = self.data_preprocessor.preprocess_data(unprocessed_training_data=sampled_training_data)
...
if __name__ == '__main__':
    ...
    file_path = config.get_next_data_path(data_type='ForexData', training_data_version='12')
    train_test = TrainingPipeline(file_path=file_path)
    train_test.run(sample_size=4)
