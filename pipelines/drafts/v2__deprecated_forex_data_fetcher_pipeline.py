...
from ForexMastermind.ML.tools import DataFrameSaver, GetADay
# Pipes
...
from ForexMastermind.ML.pipelines.pipeline_news_data_merger import TradingNewsDataMergerPipeline
...
class ForexDataFetcher:
    """
      ...
      """
    def __init__(self, pair, start_date, end_date, api_key):
    ...
    def fetch_data_for_pair(self, pair, start, end):
        """
        Fetches the historical forex data.

        Returns:
            DataFrame: A Pandas DataFrame containing the fetched forex data.
        """

        #print(f'fetching for dates{start} to {end}')
        #print(f"https://financialmodelingprep.com/api/v3/historical-chart/1min/{pair}?from={start}&to={end}&apikey={self.api_key}")
...
class FetchedDataProcessing:
    ...
    def preprocess_forex_data(self):
        ...
        preprocessed_forex = preprocessing.process()
    ...
    def preprocess(self, forex_df, econ_df):
        ...
        preprocessing = FeatureEngineeringPipeline(data=forex_df, live_data=self.live_data)
        preprocessed_forex = preprocessing.process()
...
class FetchAndProcessForexData:
    ...
    def fetch_and_preprocess_pair_data(self, pair, econ_calendar_data):
        ...
        for start, end in tqdm(date_range_generator.generate_date_ranges(self.start_date, self.end_date, timedelta(days=30)),
            ...
            forex_data = forex_data_fetcher.fetch_data_forForexDataFetcher(pair, start.strftime('%Y-%m-%d'), end.strftime('%Y-%m-%d'))
...
class ForexNewsFetcher:
    ...
    def fetch_forex_news_data(self):
        ...
        processed_news_data = TradingNewsDataMergerPipeline(start_date=self.start_date, end_date=self.end_date)
        processed_news_data.process()
